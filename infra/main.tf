...
resource "aws_lambda_function" "myfunc" {                       
  ...
  function_name    = "cloudresume-test-api"                     # Name of the Lambda function
  role             = aws_iam_role.iam_for_lambda.arn            # IAM role that grants permissions to Lambda
  handler          = "lambda_function.lambda_handler"           # Entry point for the function (Python function)
  runtime          = "python3.14"                               # Specifies Python version for execution
}
...
resource "aws_lambda_function_url" "url1" {
  ...
  cors {
    ...
    allow_origins     = ["https://resume.alfredcm.cloud"]                 # Restrict access to your website domain
